...
class Ronit:
    ...
    def help(*args):
        if args == ():
            print('all the available functions in "Ronit" are: \n Ronit.write = write the text that you input \n Ronit.geo = calculate the area or the circumference of a shape' )
            print(' Ronit.LCM(number, second number) = calculate the least common multiplier')
            print(' Ronit.GCD(number, second number) = calculate the greater common divider')
            print(' Ronit.sort(list) = returning the list that you input but sorted')
            print(' Ronit.ND(number) = Shows all the natural dividers of a range of numbers')
            print(' Ronit.rand(number, second number) = generate a random number ')
            print(' Ronit.calculate([+,-,*,/], all the numbers)')
            print(' Ronit.screenshot = take a screenshot of your screen!')
            Ronit.write(' Ronit.DrawCube(x, y, width, height, color) = opens a new window and draws a cube')

        else:
            print("TypeError: when you are typing Ronit.help() you cant enter parameters")
    def RIP(*args):
...
#     Ronit.type("all the functions are:", Ronit.Colors.BLACK(), Ronit.Colors.WHITE(), screen, 400,300)
